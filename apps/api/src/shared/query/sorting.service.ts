...
export class SortingService {
  sort<T extends Record<string, unknown>>(items: T[], sortBy?: string, sortOrder: SortOrder = 'asc') {
    if (!sortBy) {
      return [...items];
    }
    ...
    return [...items].sort((left, right) => direction * this.compareValues(left[sortBy], right[sortBy]));
  }
